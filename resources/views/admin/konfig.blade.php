                        ...
                        @foreach($listData as $key => $value)
                            ...
                            @else
                                <a class="btn btn-sm btn-secondary"
                                    href="{{route('konfig', ['tahun' => $value->tahun])}}">{{$value->tahun}}</a>
                            @endif
                        @endforeach
                    @endif
                </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control form-control-sm" id="tahun" name="tahun"
                                value="{{isset($data) ? $data->tahun : old('tahun')}}" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="tgl_buka" class="col-sm-2 col-form-label">Tanggal Buka</label>
                        <div class="col-sm-4">
                            <input type="datetime-local" class="form-control form-control-sm" id="tgl_buka" name="tgl_buka"
                                value="{{isset($data) ? $data->tgl_buka->format('Y-m-d H:i:s') : old('tgl_buka')}}" step="1"
                                required>
                        </div>

                        <label for="tgl_tutup" class="col-sm-2 col-form-label">Tanggal Tutup</label>
                        <div class="col-sm-4">
                            <input type="datetime-local" class="form-control form-control-sm" id="tgl_tutup"
                                name="tgl_tutup"
                                value="{{isset($data) ? $data->tgl_tutup->format('Y-m-d H:i:s') : old('tgl_tutup')}}"
                                step="1" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                    ...
                    <div class="row mb-3">
                        <div class="col-sm-10 offset-sm-2">
                            ...
                            @if($next == 'update' && $data->aktif == 0)
                                <a href="{{route('konfig.destroy', ['id' => $data->id])}}" class="btn btn-sm btn-danger"
