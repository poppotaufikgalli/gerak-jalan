...
<div class="container-fluid px-4">
                ...
                <div class="row">
                    <div class="mb-3 col-md-3 col-sm-12">
                        @if(Auth::user()->gid == 1)
                            ...
                            <input type="{{$domWaktu[$data->id_lomba][0]}}" class="form-control" id="tanggal_start"
                                name="tanggal_start" value="{{$domWaktu[$data->id_lomba][1]}}">
                            <input type="time" class="form-control" id="waktu_start" name="waktu_start"
                                value="{{isset($data) ? $data->waktu_start?->format('H:i:s') : old('waktu_start')}}"
                                step="1">
                        @elseif(Auth::user()->gid == 3)
                            <label for="waktu_start" class="form-label">Waktu Start</label>
                            <input type="{{$domWaktu[$data->id_lomba][0]}}" class="form-control" id="tanggal_start"
                                name="tanggal_start" value="{{$domWaktu[$data->id_lomba][1]}}" {{isset($data) && $data->waktu_start != "" ? 'readonly' : ''}}>
                            <input type="time" class="form-control" id="waktu_start" name="waktu_start"
                                value="{{isset($data) ? $data->waktu_start?->format('H:i:s') : old('waktu_start')}}"
                                step="1" {{isset($data) && $data->waktu_start != "" ? 'readonly' : ''}}>
                        @else
                            <input type="{{$domWaktu[$data->id_lomba][0]}}" class="form-control" id="tanggal_start"
                                name="tanggal_start" value="{{$domWaktu[$data->id_lomba][1]}}" readonly>
                            <input type="hidden" class="form-control" id="waktu_start" name="waktu_start"
                                value="{{isset($data) ? $data->waktu_start?->format('H:i:s') : old('waktu_start')}}"
                                step="1" readonly>
                        @endif
                    </div>
                    <div class="mb-3 col-md-3 col-sm-12">
                        @if(Auth::user()->gid == 1)
                            ...
                            <input type="{{$domWaktu[$data->id_lomba][0]}}" class="form-control" id="tanggal_finish"
                                name="tanggal_finish" value="{{$domWaktu[$data->id_lomba][2]}}">
                    ...
                    <input type="hidden" class="form-control" id="waktu_tempuh" name="waktu_tempuh"
                        value="{{isset($data) ? $data->waktu_tempuh : ''}}" step="1" readonly>
                    <input type="hidden" class="form-control" id="waktu_referensi" name="waktu_referensi"
                        value="{{ $waktu_referensi }}" readonly>
                    @if(Auth::user()->gid == 1)
                        <div class="mb-3 col-md-2 col-sm-12">
                            <label for="waktu_tempuh" class="form-label">Waktu Tempuh</label>

                            <input type="text" class="form-control" id="waktu_tempuh_1" name="waktu_tempuh_1"
                                value="{{isset($data) && $data->waktu_tempuh != null ? gmdate('H:i:s', $data->waktu_tempuh) : ''}}"
                                step="1" disabled>
                        </div>

                        <div class="mb-3 col-md-2 col-sm-12">
                            <label for="waktu_referensi" class="form-label">Waktu Referensi</label>
                            <input type="text" class="form-control" id="waktu_referensi_1" name="waktu_referensi_1"
                                value="{{ $waktu_referensi_1 }}" disabled>
                        </div>
                        ...
                        <div class="mb-3 col-md-1 col-sm-12">
                            ...
                            <input type="number" class="form-control" id="nilai_waktu" name="nilai_waktu"
                                value="{{$penilaian[1][0] ?? ''}}" disabled>
                        </div>
                    @endif
                </div>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form method="POST" action="{{route('penilaian.' . $next . '.etape')}}">
                @csrf
                <input type="hidden" name="id" value="{{$data->id}}">
                <input type="hidden" name="id_juri" value="0">
                <input type="hidden" name="id_nilai" value="5">
                <div class="row row-cols-1 row-cols-md-3 g-4 mb-4">
                    @php($etape = json_decode($data->etape, true))
                    @for($i = 1; $i <= $jml_etape; $i++)
                        <div class="col border">
                            <label class="form-label">Etape {{$i}}</label>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="etape[{{$i}}]" id="etape_{{$i}}_lewat"
                                    value="1" required {{isset($etape[$i]) && $etape[$i] == "1" ? 'checked' : ''}}>
                                <label class="form-check-label" for="etape_{{$i}}_lewat">
                                    Lewat
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="etape[{{$i}}]" id="etape_{{$i}}_taklewat"
                                    value="-1" required {{isset($etape[$i]) && $etape[$i] == "-1" ? 'checked' : ''}}>
                                <label class="form-check-label" for="etape_{{$i}}_taklewat">
                                    Tidak Lewat
                                </label>
                            </div>
                        </div>
                        ...
                        <form method="POST" action="{{route('penilaian.' . $next . '.pos')}}">
                            ...
                            <div class="row row-cols-3">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="nilai_2" name="nilai_2"
                                        value="{{$penilaian[2][$value1->juri->id] ?? ''}}" min="0" max="100" step="10"
                                        {{isset($penilaian[2][$value1->juri->id]) && $penilaian[2][$value1->juri->id] != "" ? (Auth::user()->gid == 1 ? '' : 'disabled') : ''}}>
                                </div>
                                <div class="mb-3">
                                    <label for="nilai_3" class="form-label">Kerapian</label>
                                    <input type="number" class="form-control" id="nilai_3" name="nilai_3"
                                        value="{{$penilaian[3][$value1->juri->id] ?? ''}}" min="0" max="100" step="10"
                                        {{isset($penilaian[3][$value1->juri->id]) && $penilaian[3][$value1->juri->id] != "" ? (Auth::user()->gid == 1 ? '' : 'disabled') : ''}}>
                                </div>
                                <div class="mb-3">
                                    <label for="nilai_4" class="form-label">Semangat</label>
                                    <input type="number" class="form-control" id="nilai_4" name="nilai_4"
                                        value="{{$penilaian[4][$value1->juri->id] ?? ''}}" min="0" max="100" step="10"
                                        {{isset($penilaian[4][$value1->juri->id]) && $penilaian[4][$value1->juri->id] != "" ? (Auth::user()->gid == 1 ? '' : 'disabled') : ''}}>
                                </div>
                            </div>
                            <button type="submit" class="btn btn-sm btn-primary">Simpan</button>
                        </form>
